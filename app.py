    ...
    with ui.column().classes('w-full max-w-6xl mx-auto p-4 md:p-8 gap-4'):
        ui.add_head_html('''<style>
            ...
            @media (max-width: 768px) {
                .q-field__label, .q-field__native, .q-checkbox__label { font-size: 0.9rem !important; }
        ...
        ui.label("Deutsches Vermögens- & Rentenprojektions-Tool").classes('text-xl md:text-3xl font-extrabold text-gray-800 dark:text-gray-200 mb-2')
        ui.label("Dieses Tool modelliert den Aufbau und die Entnahme Ihrer Säulen der Altersvorsorge. Es berücksichtigt unter anderem die Vorabpauschale, das Halbeinkünfteverfahren (12/62) und den Unterschied zwischen KVdR und freiwilliger GKV.").classes('text-sm md:text-base text-gray-600 dark:text-gray-400 mb-4')
        
        # Settings Section
        ui.label("⚙️ Einstellungen & Parameter").classes('text-xl md:text-2xl font-bold mt-4 border-b-2 border-gray-200 pb-2 w-full')
        ...
        with ui.row().classes('w-full items-center bg-gray-100 dark:bg-slate-800 p-4 rounded-lg shadow-inner justify-between mb-4 flex-wrap gap-4'):
            with ui.row().classes('items-center gap-4'):
                ui.label("📂 Parameter verwalten:").classes('font-bold text-sm md:text-base text-gray-700 dark:text-gray-300')
            ...
            ui.button("💾 Aktuelle Werte speichern", on_click=download_params, color='secondary').classes('shadow-sm text-sm md:text-base')

        # Input Grid
        with ui.grid(columns=1).classes('w-full gap-6 md:grid-cols-2 lg:grid-cols-3'):
            with ui.expansion('1. Persönliche Daten', icon='person').classes('w-full bg-gray-100 dark:bg-gray-800 shadow-md rounded-md text-black dark:text-white').props('default-opened header-class="bg-gray-200 dark:bg-gray-900 text-base md:text-lg font-bold text-blue-900 dark:text-blue-100"'):
            ...
            with ui.expansion('3. Aktienmarkt (Depot)', icon='show_chart').classes('w-full bg-gray-100 dark:bg-gray-800 shadow-md rounded-md text-black dark:text-white').props('default-opened header-class="bg-gray-200 dark:bg-gray-900 text-base md:text-lg font-bold text-blue-900 dark:text-blue-100"'):
            ...
            with ui.expansion('4. Private Rente (Schicht 3)', icon='savings').classes('w-full bg-gray-100 dark:bg-gray-800 shadow-md rounded-md text-black dark:text-white').props('default-opened header-class="bg-gray-200 dark:bg-gray-900 text-base md:text-lg font-bold text-blue-900 dark:text-blue-100"'):
        ...
        def run_simulation():
            ...
            with results_container:
                ui.label("Ergebnisse werden berechnet...").classes('text-base md:text-lg text-gray-500 dark:text-gray-400 italic')
            ...
            try:
                ...
                results_container.clear()
                with results_container:
                    ui.markdown(f"**💡 Info zur Inflationsanpassung:** Die Simulation geht davon aus, dass Sie Ihre monatlichen Sparraten jährlich um die Inflation ({state.inflation}%) erhöhen. Möchten Sie Ihre Sparrate stattdessen konstant halten, müssten Sie von Beginn an **{stock_flat:,.0f} € ins Depot** und **{priv_flat:,.0f} € in die private Rente** sparen, um exakt dasselbe nominale Endkapital zu Rentenbeginn zu erreichen.").classes('bg-blue-50 dark:bg-blue-900 p-4 rounded-lg shadow-inner text-blue-900 dark:text-blue-200 border border-blue-200 dark:border-blue-800 mb-6 text-sm md:text-base')
                    ...
                    ui.label("Vermögensentwicklung & Entnahme (Kaufkraftbereinigt)").classes('text-xl md:text-2xl font-bold mt-8')
                    ui.label("Dieses Diagramm zeigt Ihr inflationsbereinigtes Kapital und stellt die tatsächliche heutige Kaufkraft Ihres Geldes dar.").classes('text-sm md:text-base text-gray-600 dark:text-gray-400 mb-4')
                    ...
                    }).classes('w-full h-96 text-sm md:text-base')
                ...
                results_container.clear()
                with results_container:
                    ui.label(f"Fehler bei der Berechnung: {str(e)}").classes('text-red-500 dark:text-red-400 font-bold p-4 text-sm md:text-base')

        ui.button("🚀 Simulation starten", on_click=run_simulation, color='primary').classes('w-full mt-6 py-3 md:py-4 text-lg md:text-xl font-bold shadow-lg')

        results_container = ui.column().classes('w-full mt-4')

        with ui.expansion('⚠️ Disclaimer (Haftungsausschluss)', icon='warning').classes('w-full bg-yellow-50 dark:bg-yellow-900 border border-yellow-200 dark:border-yellow-800 mt-8 text-black dark:text-white').props('header-class="text-base md:text-lg font-bold"'):
            ui.markdown("""
        ...
        with ui.expansion('ℹ️ INFO: Berechnungs- und Steuerdetails anzeigen', icon='info').classes('w-full !bg-gray-100 dark:!bg-gray-800 shadow-md rounded-md text-black dark:text-white mb-6').props('header-class="!bg-gray-200 dark:!bg-gray-900 text-base md:text-lg font-bold text-blue-900 dark:!text-blue-100"'):
